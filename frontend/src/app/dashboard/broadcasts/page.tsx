    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-neutral-900 flex items-center tracking-tight">
            <History className="h-8 w-8 text-neutral-850 mr-3" />
            Broadcast History
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white border border-neutral-200 rounded-2xl p-6 relative overflow-hidden shadow-sm">
          <div className="absolute top-4 right-4 bg-neutral-100 p-2.5 rounded-xl border border-neutral-250">
            <Megaphone className="h-5 w-5 text-neutral-700" />
          </div>
          <p className="text-sm font-medium text-neutral-500">Total Campaigns</p>
          <p className="text-3xl font-bold text-neutral-900 mt-2">{totalCampaigns}</p>
          <div className="mt-2 text-xs text-neutral-500">Scheduled & executed batches</div>
        </div>

        {/* Card 2: Total Messages Sent */}
        <div className="bg-white border border-neutral-200 rounded-2xl p-6 relative overflow-hidden shadow-sm">
          <div className="absolute top-4 right-4 bg-emerald-50 p-2.5 rounded-xl border border-emerald-200">
            <MessageSquare className="h-5 w-5 text-emerald-800" />
          </div>
          <p className="text-sm font-medium text-neutral-500">Messages Dispatched</p>
          <p className="text-3xl font-bold text-neutral-900 mt-2">{totalSent.toLocaleString()}</p>
          <div className="mt-2 text-xs text-neutral-500">Delivered to active numbers</div>
        </div>
        ...
        <div className="bg-white border border-neutral-200 rounded-2xl p-6 relative overflow-hidden shadow-sm">
          ...
          <p className="text-sm font-medium text-neutral-500">Average Delivery Rate</p>
          <p className="text-3xl font-bold text-neutral-900 mt-2">{averageSuccessRate}%</p>
          <div className="mt-2 text-xs text-neutral-500">Overall success metrics</div>
        </div>
      </div>
